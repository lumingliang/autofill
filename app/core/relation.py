...
class RelationQuery:
    ...
    async def get_user_menu_ids(user_id: int, tenant_id: int | None) -> Set[int]:
        ...
        target_role_ids = [r.id for r in roles if tenant_id and r.tenant_id == tenant_id]
        if not target_role_ids:
            return set()
        rows = await RoleMenu.filter(role_id__in=target_role_ids).values("menu_id")
        return {r["menu_id"] for r in rows}

    @staticmethod
    async def get_user_api_ids(user_id: int, tenant_id: int | None) -> Set[int]:
        """获取用户在指定租户下的所有API ID集合"""
        role_ids = await RelationQuery.get_role_ids_by_user_id(user_id)
        if not role_ids:
            return set()
        roles = await Role.filter(id__in=role_ids).all()
        ...
        if not target_role_ids:
            return set()
        rows = await RoleApi.filter(role_id__in=target_role_ids).values("api_id")
        return {r["api_id"] for r in rows}

    @staticmethod
    async def get_user_api_permissions(user_id: int, tenant_id: int | None) -> Set[Tuple[str, str]]:
        """获取用户在指定租户下的所有API权限 (method, path) 集合"""
        api_ids = await RelationQuery.get_user_api_ids(user_id, tenant_id)
        if not api_ids:
            return set()
        rows = await Api.filter(id__in=api_ids).values("method", "path")
