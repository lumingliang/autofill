...
class RunCommandInput(BaseModel):
    ...
    cwd: Optional[str] = Field(default=None, description="The working directory to run the command in, the value MUST be absolute path, if not provided, it will be the current working directory.")
    ...
    command_type: Optional[str] = Field(default=None, description="Classify the command type BEFORE deciding [blocking]. Available types: [web_server, long_running_process, short_running_process, other].\n")
    target_terminal: Optional[str] = Field(default=None, description="The target terminal for command execution. Can be a terminal id (from <available_terminal/> or previous <toolcall_result/>), or 'new' to create a new terminal. If not specified, the system will automatically reuse an idle AI terminal or create a new one if none is available. For performance, prefer reusing existing idle terminals.")
