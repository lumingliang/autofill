...
class RunCommandInput(BaseModel):
    ...
    cwd: Optional[str] = Field(default=None, description="The working directory to run the command in, the value MUST be absolute path, if\nnot provided, it will be the current working directory.")
    blocking: bool = Field(description="Set to `false` ONLY for web_server or long_running_process command types. Set to\n`true` for all other cases.\n")
    command_type: Optional[str] = Field(default=None, description="Classify the command type BEFORE deciding [blocking]. Available types:\n[web_server, long_running_process, short_running_process, other].\n")
    target_terminal: Optional[str] = Field(default=None, description="The target terminal for command execution. Can be a terminal id (from\n<available_terminal/> or previous <toolcall_result/>), or 'new' to create a new\nterminal. If not specified, the system will automatically reuse an idle AI\nterminal or create a new one if none is available. For performance, prefer\nreusing existing idle terminals.")
    requires_approval: bool = Field(description="Whether the user must approval the command before it is executed. Set to 'false'\nfor safe operations like read/write files/directories, create/initialize/build\nprojects, install project dependencies, running development servers.\n")
