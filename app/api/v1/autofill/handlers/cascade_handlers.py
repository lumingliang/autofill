...
async def create_cascade_config(data: dict, token: str = Header(...)) -> Success:
    ...
    try:
        result = await field_cascade_service.create_cascade_config(
            ...
            field_label_suffix=data.get("field_label_suffix", "-的二三级"),
...
async def list_cascade_configs(parent_field_id: int = Query(0), token: str = Header(...)) -> Success:
    user = await AuthControl.is_authed(token)
...
async def delete_cascade_config(config_id: int = Query(...), token: str = Header(...)) -> Success:
    user = await AuthControl.is_authed(token)
...
async def list_cascade_data(config_id: int = Query(...), token: str = Header(...)) -> Success:
    user = await AuthControl.is_authed(token)
