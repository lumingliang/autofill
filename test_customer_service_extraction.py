...
# 用户提供的客服对话
customer_dialogue = """
张先生: 你好，我的车最近充电老是充到80%就停了，而且底盘偶尔有咯噔咯噔的响声，这怎么回事？
...
def print_schema():
    """打印 function schema"""
    print("=" * 80)
    print("客服对话结构化提取 - Function Schema")
    print("=" * 80)
    print(json.dumps(function_schema, ensure_ascii=False, indent=2))


def print_test_request():
    """打印测试请求示例"""
    request_body = {
    ...
    print("\n" + "=" * 80)
    print("测试请求示例 (POST /api/llm/proxy)")
    print("=" * 80)
...
def print_expected_output():
    ...
    expected_output = {
        ...
        "vehicle_info": {
            "model": "2024款极越01超长续航版",
            "purchase_date": "2023年11月",
            "mileage": "1.2万公里",
            "license_plate": "粤A12345D",
            "vin_last6": "253718"
        },
        "issues": [
            {
                "type": "充电问题",
                "description": "充电到80%就停止",
                "symptoms": [
                    "快充到80%就跳枪",
                    "慢充偶尔能充到100%，最近几次也停在80%"
                ],
                "error_message": "高压系统异常，请联系服务商"
            },
            {
                "type": "底盘异响",
                "description": "底盘偶尔有咯噔咯噔的响声",
                "symptoms": [
                    "低速过减速带时有异响",
                    "颠簸路面时有异响",
                    "速度起来后消失"
        ...
        "appointment": {
            "service_center": "极越授权服务中心",
            "address": "广州天河中路",
            "appointment_time": "明天下午14:00",
            "required_documents": [
                "车主本人身份证",
                "行驶证",
                "购车发票（或电子发票）"
            ],
            "estimated_duration": "常规检测约1小时，如需更换部件或软件升级当天完成"
        },
        "service_satisfaction": {
            "urgency_level": "一般",
            "customer_emotion": "一般"
...
if __name__ == "__main__":
    ...
    print("=" * 80)
    print("""
1. 首先确保已在系统中添加魔搭社区的LLM配置
2. 使用以下curl命令测试代理接口:

   curl -X POST http://localhost:9999/api/llm/proxy \\
