...
2. **结构化输出**: 强制使用 `with_structured_output()` 方法，不手动解析 JSON
3. **Pydantic 模型**: 从 json_sample 自动生成 Pydantic Model，作为 LangChain 的结构化输出类型
...
class LangChainModelFactory:
    """LangChain 模型工厂 - 根据配置创建对应的 ChatModel"""
    
    # 提供商到 LangChain 模型类的映射
    PROVIDER_MAP = {
        "openai": ChatOpenAI,
    ...
    async def create_chat_model(
        ...
        """
        # 获取对应的模型类
        model_class = cls.PROVIDER_MAP.get(config.provider)
        if not model_class:
...
class LLMService:
    ...
    async def chat(
        ...
        """
        # 查询模型配置（租户隔离 + 全局）
        config = await cls._get_config(model_code, tenant_id)
        if not config:
